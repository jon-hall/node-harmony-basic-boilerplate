        ...
        return function() {
            ...
            if(isGeneratorFn(fn)) {
                outArgs[fnArg] = function(done) {
                    suspend.run(fn, function(err) {
                        if(err) {
                            if(err.stack) {
                                // Jasmine doesn't seem to display the stack - so put
                                // it in the message to be reported with the result
                                err.message = err.stack;
                            }
                            // Grab the current top-level Suite and report the
                            // exception directly to it (which relays it to
                            // children suites/specs)
                            jasmine.getEnv().topSuite().onException(err);
                        }
                        done();
                    });
                };
